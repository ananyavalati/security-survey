...
import { QUESTIONS, SCALE, CATEGORIES } from "../securitySurvey/questions";
...
export default function SurveyPage() {
  ...
  return (
    <div style={{ maxWidth: 840, margin: "40px auto", padding: 16 }}>
      ...
      <p>Answer from {SCALE.labels[0]} (1) to {SCALE.labels[4]} (5).</p>
      <p>Progress: {answeredCount}/{QUESTIONS.length}</p>

      <form onSubmit={onSubmit}>
        {byCat.map(({cat, items}) => (
          <fieldset key={cat} style={{ border: "1px solid #eee", borderRadius: 8, padding: 16, marginBottom: 16 }}>
            ...
            {items.map(q => (
              <div key={q.id} style={{ padding: "12px 8px", borderBottom: "1px solid #f6f6f6" }}>
                ...
                <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
                  {[1,2,3,4,5].map(val => (
                    <label key={val} style={{ display: "inline-flex", gap: 6, alignItems: "center" }}>
                      <input
                        type="radio"
                        name={q.id}
                        value={val}
                        ...
                        required
                      />
                      <span style={{ fontSize: 13 }}>{val} – {SCALE.labels[val-1]}</span>
                    </label>
                  ))}
                </div>
              </div>
